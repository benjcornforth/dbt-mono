...
In a multi-domain project, this task is invoked once per domain.
The `forge.domain` Spark conf tells it which domain to populate.
"""
...
def main():
    ...
    print(f"✅ {task.profile_name}: ingest_from_volume [{domain}] — ingested {ingested} files")
