...
def _seed_inline_rows(seed_name: str, seed_def: dict) -> list[dict[str, Any]] | None:
    """Return inline seed rows when authored directly in DDL YAML."""
    rows = seed_def.get("rows")
    if rows is None:
        return None
    if not isinstance(rows, list):
        raise ValueError(f"Seed '{seed_name}' rows must be a list of mappings")
    normalized: list[dict[str, Any]] = []
    for index, row in enumerate(rows, start=1):
        if not isinstance(row, dict):
            raise ValueError(f"Seed '{seed_name}' row {index} must be a mapping")
...
def _compile_setup_model_sql(
    ...
    *,
    ...
    """Compile setup-time DDL for models whose physical tables must already exist."""
    ...
    lines.append("-- Setup DDL: create relation before process tasks run.")
    lines.append("-- Generated by: forge compile --pure-sql")
    lines.append("")
...
def compile_pure_sql_seed(
    ...
    Reads inline YAML rows or a CSV file and emits INSERT VALUES rows so
    the seed data lives entirely in SQL — no dbt seed required.
    """
    import csv as csv_mod

    columns = seed_def.get("columns", {})
    fq_table = f"`{catalog}`.`{schema}`.`{seed_name}`"
    inline_rows = _seed_inline_rows(seed_name, seed_def)
    source_label = "inline YAML rows" if inline_rows is not None else str(csv_path)
    ...
    # TRUNCATE + INSERT from inline rows or CSV
    ...
    def _format_seed_value(raw: Any, ctype: str, *, strip_strings: bool) -> str:
        if raw is None:
            return "NULL"
        if isinstance(raw, str):
            raw = raw.strip() if strip_strings else raw
    ...
    if inline_rows is not None:
        for row in inline_rows:
            vals = [
                _format_seed_value(row.get(col_name), col_types.get(col_name, "STRING"), strip_strings=False)
        ...
        with open(csv_path, newline="") as f:
            ...
            for row in reader:
                vals = [
                    _format_seed_value(row.get(col_name, ""), col_types.get(col_name, "STRING"), strip_strings=True)
                    for col_name in col_names
                ]
                value_rows.append(f"  ({', '.join(vals)})")
...
def _ensure_runtime_macros() -> None:
    """Materialize Forge-managed dbt macros into the hidden generated path."""
...
def compile_all(
    ...
    # Materialize Forge runtime macros into the hidden generated path.
    _ensure_runtime_macros()
...
def compile_pure_sql_model(
    ...
    Tables: TRUNCATE + INSERT INTO, with relation DDL emitted separately in setup.
    ...
    if materialized == "view":
        ...
        # Table: relation DDL lives in SETUP; PROCESS only refreshes data.
...
def compile_all_pure_sql(
    ...
    # Step 1b: Forge-managed seeds (inline YAML rows or CSV-backed authored seeds)
    seeds = load_seeds(ddl_path, forge_config=forge_config)
    for seed_name, seed_def in seeds.items():
        ...
        csv_path: Path | None = None
        if _seed_inline_rows(seed_name, seed_def) is None:
    ...
    # Step 2: Setup relation DDL for physical models
    for model_name in ordered:
        model_def = models[model_name]
        if _is_ephemeral_model(model_def):
            continue
        materialized = model_def.get("materialized", "view") if isinstance(model_def, dict) else "view"
        if materialized not in ("table", "incremental") and not model_def.get("managed_by"):
            continue
        m_catalog, m_schema = model_locations[model_name]
        setup_sql = _compile_setup_model_sql(
            model_name,
            model_def,
            m_catalog,
            m_schema,
        ...
        setup_path = phase_dirs["setup"] / f"{seq:03d}_{model_name}.sql"
        setup_path.write_text(setup_sql)
        results[f"_setup_{model_name}"] = setup_path
        seq += 1

    # Step 3..N: Process models in dependency order
    for model_name in ordered:
        model_def = models[model_name]
        if _is_ephemeral_model(model_def):
            continue
        if model_def.get("managed_by"):
            continue
        m_catalog, m_schema = model_locations[model_name]
...
def compile_teardown_sql(
    ...
    """
    prof = profile or {}
    teardown_cfg = (forge_config or {}).get("teardown", {}) if isinstance((forge_config or {}).get("teardown", {}), dict) else {}
    teardown_mode = teardown_cfg.get("mode", "object_level")
    if teardown_mode not in {"object_level", "schema_cascade"}:
        raise RuntimeError(
            "Compile error: forge.yml teardown.mode must be one of "
            "['object_level', 'schema_cascade']."
        )
    ...
    if teardown_mode == "schema_cascade":
        schema_targets: set[tuple[str, str]] = set()
        for model_name, model_def in models.items():
            if _is_ephemeral_model(model_def):
                continue
            schema_targets.add(model_locations[model_name])
        ...
        for udf_name, udf_def in udfs.items():
            ...
            schema_targets.add((udf_catalog, udf_schema))

        schema_targets.add((meta_cat, meta_sch))

        for cat, sch in sorted(schema_targets):
            lines.append(f"DROP SCHEMA IF EXISTS `{cat}`.`{sch}` CASCADE;")
        lines.append("")
    else:
...
def _validate_sql_schema_ordering(output_dir: Path) -> None:
    ...
    CREATE SCHEMA IF NOT EXISTS in the same file or in a setup artifact.

    Setup artifacts may establish schemas for later process tasks. Outside of
    that setup/process contract, each SQL file still needs to be self-sufficient.
    ...
    errors: list[str] = []
    setup_schemas: set[tuple[str, str]] = set()

    setup_dir = _phase_sql_dirs(output_dir).get("setup")
    if setup_dir and setup_dir.exists():
        for sql_file in sorted(setup_dir.rglob("*.sql")):
            content = sql_file.read_text()
            for m in create_pattern.finditer(content):
                setup_schemas.add((m.group(1), m.group(2)))
    ...
    for sql_file in sorted(output_dir.rglob("*.sql")):
        ...
        for line in content.split("\n"):
            ...
            for m in ref_pattern.finditer(line):
                ...
                if ref not in file_schemas and ref not in setup_schemas:
                    errors.append(
                        f"  {sql_file.name}: DDL references `{ref[0]}`.`{ref[1]}` "
                        f"but neither this file nor setup artifacts create that schema"
                    )
    ...
    if errors:
        ...
        raise RuntimeError(
            ...
            "Each SQL file must create its own schema unless the schema is established in setup/.\n"
            + "\n".join(errors)
            + "\n\nAdd CREATE SCHEMA IF NOT EXISTS to the file or emit the schema in setup/."
        )
