...
# 3. Build the staged target bundle
poetry run forge build --target dev

# 4. Deploy the staged bundle
cd artifacts/targets/dev && databricks bundle deploy --target dev
```
...
poetry run forge build --target dev && cd artifacts/targets/dev && databricks bundle deploy --target dev
...
forge graph -p dev    # current asset graph as Mermaid
forge graph -p dev -o docs/table-lineage.md  # GitHub-renderable Markdown
...
```bash
forge compile --profile dev --pure-sql   # emits phased SQL files to artifacts/targets/dev/sql/
```

Produces standalone SQL that runs directly on a Databricks SQL warehouse — no dbt installation, no Python, no cold starts. Files are split into setup, process, and teardown phases under the selected target.

```
artifacts/targets/dev/sql/setup/000_udfs.sql
artifacts/targets/dev/sql/setup/001_lineage_graph.sql
artifacts/targets/dev/sql/process/001_stg_customers.sql
artifacts/targets/dev/sql/process/003_customer_clean_quarantine.sql
artifacts/targets/dev/sql/teardown/backup.sql
artifacts/targets/dev/sql/teardown/teardown.sql
```
...
Macros (lineage, quarantine, prior_version, UDFs) live in `macros/` and are staged into each target bundle automatically.
...
| `forge visual-docs [-p profile] [--column model.col]` | Regenerate pipeline, graph, provenance, and consolidated visual Markdown docs |
| `forge graph [-p profile] [-o file]` | Current asset graph as Mermaid; `.md` outputs are GitHub-renderable |
