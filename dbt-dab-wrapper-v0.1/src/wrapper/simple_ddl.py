...
#
# Then:  wrapper compile   → generates all .sql + schema.yml
#        wrapper deploy    → runs everything
#
...
#
# wrapper compile → also writes _udfs.sql
...
#
# Then:  wrapper migrate   → applies changes to models.yml + recompiles
...
def generate_agent_guide(
    ...
    Reads wrapper.yml + models.yml to produce contextual instructions
    ...
    schedule = wrapper_config.get("schedule")
    ...
    lines.append("")
    lines.append("This guide was auto-generated by `wrapper guide`.")
    ...
    lines.append("|---------|-------------|")
    lines.append("| `wrapper setup` | Creates project structure + wrapper.yml |")
    lines.append("| `wrapper compile` | Turns models.yml into SQL (no coding needed) |")
    lines.append("| `wrapper deploy` | Builds + deploys to Databricks |")
    lines.append("| `wrapper diff` | Shows what changed since last deploy |")
    lines.append("| `wrapper diff --mermaid` | Visual diagram of changes (color-coded) |")
    lines.append("| `wrapper explain model.col` | Full provenance tree for any column |")
    lines.append("| `wrapper explain model.col --mermaid` | Provenance as Mermaid diagram |")
    lines.append("| `wrapper explain model.col --json` | Provenance as JSON (for scripting) |")
    lines.append("| `wrapper workflow` | Shows the pipeline DAG |")
    lines.append("| `wrapper workflow --mermaid` | Visual diagram of the pipeline |")
    lines.append("| `wrapper migrate` | Applies migration YAML files |")
    lines.append("| `wrapper udfs` | Shows defined UDFs |")
    lines.append("| `wrapper validate` | Runs data quality checks |")
    lines.append("| `wrapper codegen` | Generates type-safe Python SDK |")
    lines.append("| `wrapper guide` | Regenerates this guide |")
    lines.append("| `wrapper teardown` | Safely destroys everything |")
    lines.append("")
    ...
    lines.append("wrapper explain customer_summary.total_revenue")
    lines.append("")
    lines.append("# As Mermaid diagram")
    lines.append("wrapper explain customer_summary.total_revenue --mermaid --output docs/explain.mmd")
    lines.append("")
    lines.append("# As JSON (for scripting)")
    lines.append("wrapper explain customer_summary.total_revenue --json")
    lines.append("")
    lines.append("# Full detail (includes upstream checks)")
    lines.append("wrapper explain customer_summary.total_revenue --full")
    lines.append("```")
    lines.append("")
    ...
    lines.append("| Command | What it does |")
    lines.append("|---------|-------------|")
    lines.append("| `wrapper udfs` | Shows all defined UDFs |")
    lines.append("| `wrapper udfs --output udfs.sql` | Writes CREATE FUNCTION SQL |")
    lines.append("| `wrapper compile` | Auto-generates `_udfs.sql` alongside models |")
    lines.append("")
    ...
    lines.append("```")
    lines.append("")
    lines.append("Then run: `wrapper migrate`")
    lines.append("")
    ...
    lines.append("|---------|----------|")
    lines.append("| `wrapper.yml not found` | Run `wrapper setup` first |")
    lines.append("| Models not updating | Run `wrapper compile` after editing models.yml |")
    ...
    lines.append("| Want to see the pipeline | Run `wrapper workflow --mermaid` |")
    lines.append("| Python type errors | Run `wrapper codegen` to regenerate SDK |")
    lines.append("")
